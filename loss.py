...
class BerHu(nn.Module):
    ...
    def forward(depth_n, output):
        mask = depth_n > 0
        if not output.shape == depth_n.shape:
            _, _, H, W = depth_n.shape
            foutput = F.upsample(output, size=(H, W), mode='bilinear')
        output = output * mask
        diff = torch.abs(depth_n - output)
